...
> **2026-09-14 官方对齐**：`guanvis 0.1.47` 覆盖发布会重置被替换页面的草稿，避免编辑器继续打开旧卡片或脚本。覆盖前核对目标、备份及未发布编辑；发布后同时验收浏览态和编辑态。新建页面须先明确目标目录。常规流程优先用官方 publish；本篇 60004、descriptor patch 与 phoneLayout 是特定环境的历史兼容依据，不宣称已在新版重测。
