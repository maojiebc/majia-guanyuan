...
> **版本**：V3.2.0（2026-09-15）· **环境**：Node ≥20 · **前置**：官方全家桶 `npm i -g @guandata/guanskill && guanskill install-skill`（装齐 guancli / guanvis / guanetl / guanwf / guands / guanmetric + 各自 AI skill）· **认证**：`guancli auth login`（全家桶共用一套 profile，本 skill 不再单独要 config.json）· **作用域**：本地私有 BI 实例
...
>
> 🆕 **V3.2.0**（2026-09-15）：手机成绩单 V2：专供 ETL、营业额选店、冻结明细、无需大模型的异常提示与券转化。合成数据离线样本和SQL参考见 [V2案例](examples/store-mobile-scorecard-v2/README.md)。官方CLI版本不变，完整历史见 [CHANGELOG.md](CHANGELOG.md)。
