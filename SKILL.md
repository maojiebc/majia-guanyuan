...
> **版本**：V3.1.9（2026-08-19）· **环境**：Node ≥20 · **前置**：官方全家桶 `npm i -g @guandata/guanskill && guanskill install-skill`（装齐 guancli / guanvis / guanetl / guanwf / guands / guanmetric + 各自 AI skill）· **认证**：`guancli auth login`（全家桶共用一套 profile，本 skill 不再单独要 config.json）· **作用域**：本地私有 BI 实例
...
>
> 🆕 **V3.1.9**（2026-08-19）：官方全家桶 07-24 以来 9 个聚合包版本一次性对齐，`guanskill` 0.1.17→**0.1.26**（guancli 1.0.51 / guanvis 0.1.38 / guanetl 0.1.27 / guanwf 0.1.826 / guands 0.1.26 / guanmetric 0.1.8）。头条新能力：`guancli insight` 仪表板洞察 + Dashboard Agent、`guanvis live` 实时工程、`guanmetric metric-tree` 指标树 + `accelerate` 查询加速。`guanwf` `--confirm` 门禁沿用。护城河零删减。逐条变更见 [CHANGELOG.md](CHANGELOG.md)。
...
| **`majia-guanyuan`**（本 skill） | **3.1.9** | 业务实战 + 引擎级踩坑 + 方法论 | **Part B** ETL 整库治理判断 + 10 类引擎报错 + 双源字段审计 + B-17 全链路重写/ExecPlan · **Part C** 既有页自定义图表 HTML/JS 注入排障 + 固定卡/overlay · **Part C-12** HTML 应用化看板 + descriptor patch 联 dataView + **视觉设计底线（反 AI 味红线 + 五层验收）** · **Part D** v7 草稿-发布状态机绕过 + 节点化静默坑 + phoneLayout · **Part E** SuperApp 反向工程 · **AI-native ADS** 方法论 · **餐饮 BI 公式库** |
...
> 基于 `@guandata/guancli@1.0.51` 的实证记录。所有 API 路径、payload 字段、报错信息、治理判断维度均来自真实跑通的请求。覆盖整库治理扫描 + 60+ 张 ETL 创建/重构/修复/删除的实战。
>
...
>
> **架构**：v7 BI 的草稿/发布分离机制使**手撸 `/api/page` + `/api/card` 全链路废弃**；银弹是官方 `guanvis`（原 `guanvis-skill`，全家桶成员，现公网 `@guandata/guanvis@0.1.38`），`guanskill install-skill && guanvis publish .` 30 秒一键发布整个 page + custom chart + dataView，跳过所有草稿/发布的状态机。桌面端 另有 `guanvis live` 对话式路径（`live project validate/publish` 走完整 DSL；**不得用 P0 命令模拟自定义图表**，C-12 descriptor patch / 60004 / phoneLayout 仍走本 Part）。配套硬规则：CSV 散客 `会员ID` 是 `""` 不是 NULL（三态判断必须 `IS NOT NULL AND <> ''`）；STRING 字段才能 `<> ''`，日期/数字 Spark 严格类型不行；Spark CTE 别名必须英文；ETL update 必须带 `OUTPUT_DATASET.dataSource.dsId` 否则 1012；数据集上传 / 建集走官方 `guands`（`create-db` / `import` / `replace-data`，不必再 BI UI 手动）；大表 pandas 用 `to_csv` 而非 `to_excel`（50 倍速差）。
>
...
- **V3.1.7**（2026-07-12）餐饮 BI 公式库 + 咖啡连锁模拟中台迁出至独立仓库 [majia-huiyuan](https://github.com/maojiebc/majia-huiyuan)。
