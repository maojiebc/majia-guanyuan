...
> **版本**：V2.0.0（2026-05-12）· **环境**：Node ≥20 · **依赖**：`@guandata/guancli@^1.0.19` · **作用域**：本地私有 BI 实例
...
官方在 2026-05-11 通过 `guancli install-skill` 也分发了一个名为 `guancli` 的 skill，定位**只读分析 + ChatBI + 表单填报 CRUD**；写操作分流到 `guanetl-skill / guanvis-skill / guands-skill / guanexport-skill / guanadmin-skill`（这 5 个 2026-05-12 npm 实测 404，未发包，下文 Part B 入口处也有提示）。

**两个 skill 角色互补**：

- **官方 `guancli` skill** → 只读 + ChatBI L1/L2 + 表单 CRUD
- **majia-guanyuan**（本 skill）→ 建卡 / 取数 / 删卡 / ETL 治理写入 / SmartETL 全链路重写 / 自定义图表 HTML/CSS/JS 注入 / payload_json / ExecPlan / 60+ ETL 战例 + 10 类报错手册

如果同时启用，agent 在"分析 ETL 资源"这类只读场景下可能双触发；想降低歧义建议保留本 skill（命令面更广，已含 guancli 的只读路由），把官方 symlink 卸了：`rm ~/.claude/skills/guancli`（仅断 Claude Code 入口，不影响 `~/.agents/skills/guancli/` 真目录与其他 agent 加载）。
...
>
> ⚠️ 官方 guancli SKILL.md 把 BI 操作拆成 5 个兄弟 skill（`guanetl-skill` / `guanvis-skill` / `guands-skill` / `guanexport-skill` / `guanadmin-skill`），但 **2026-05-12 npm 实测全部 404，只发了 `@guandata/guancli` 本体**。在它们落地之前，ETL 写入、Card/Page 创建、数据集 CRUD 全部继续走 `guancli fetch` + 本 skill 的 Part A/B 实战手册。
...
**最新：V2.0.0** (2026-05-12) — 重命名为 `majia-guanyuan` + 对齐 `@guandata/guancli@1.0.19` 命令面（ChatBI 主题问数 / SuperApp / 多环境 auth / 连接探测）+ Node ≥20。
