...
**Latest: V2.1.10** (2026-05-21) — **`references/v7-page-card-publish-pipeline.md` §16 mobile phoneLayout complete guide + v7 draft save API dead-end** (~330 lines). Distilled from porting 9 demo dashboards to mobile in a single day (30+ rounds of API exploration): v7 BI has **NO callable draft save REST API** (8 candidate endpoints all stub/404 — `/save` 200 but GET unchanged, `/saveMeta` 404, `/save-meta` 404, `/update` 404, `/draft/save` 404, `/api/v2/.../save` 500, `/api/page/save` 500, `/phoneLayout` 500, `PUT /phoneLayout` 404); BI editor saves via non-REST channel (WebSocket / Redux internal batch). The ONLY working path: `guanvis-skill pack` → Python script inject phoneLayout into `page.meta` (**must be JSON string, not object — `json.dumps(inner, ensure_ascii=False)` or else `error.expected.jsstring` upload error**) → `guanvis-skill upload` via transfer API directly overwrites the published page. Includes: phoneLayout five-field structure (selector must wrap in `group_AUTO_PHONE` container, `mobileHeightUnit:60`, customChart `h=40 ≈ 572px` fits 4 KPI + 4 charts), CSS @media 768px/480px template (KPI 4→2 col + `html, body { height: auto }` to unlock iOS scrolling), and `scripts/inject_phone_layout.py` ready-to-use tool.

**V2.1.9** (2026-05-21) — **`references/v7-page-card-publish-pipeline.md` §15 customChart three traps + autoBootstrap + chip toolbar fallback** (~180 lines). §15.1 BI doesn't auto-call `renderChart` (race condition, same code works on 03 but hangs on 07) — must add iframe-top `autoBootstrap` 5s fallback to `POST /api/card/<cdId>/data` and manually feed `renderChart`. §15.2 selector linkage fails (autoBootstrap bypasses BI redux dispatch + 7 server-side filter body formats all rejected + `PAGE_DATA_SDK.initPage()` stuck on pgId). §15.3 ultimate fallback: chip toolbar pattern (add `addRow(f("门店类型"))` to every dataView + JS state + CSS gradient — used in 5 dashboards). §15.4 when to use selector vs chip (cardinality ≤10 strongly recommend chip). §15.5 troubleshooting checklist.

**V2.1.8** (2026-05-21) — **`references/v7-page-card-publish-pipeline.md` §14 SmartETL node-level silent traps**. From rewriting 5 flagship SmartETL demos from "one giant SQL" to 7-15 nodes: (A) **GROUP_BY node STRING `COUNT_DISTINCT` silently swapped to NUL `aggrType`, output empty** (UI shows no error, downstream JOIN all empty); (B) **JOIN_DATA node only honors `predicates[0]`**, multi-key JOIN (orderID + productID + storeID) produces 6M row Cartesian explosion; (C) **`FULL_OUTER` silently downgraded to `INNER`**; (D) **two-layer GROUP_BY workaround** required for distinct count.

**V2.1.7** (2026-05-21) — **Routing/triggering refresh (docs-only)**. Rewrote `SKILL.md` frontmatter `description` to include Part D trigger keywords (v7 BI / `60004` draft / guanvis-skill / CSV three-state / Spark Chinese CTE alias / `1012` same-name file) and V2.1.5 restaurant-BI-formulas keywords (复购率/客单价/RFM/DWD wide-table/financial dual-source reconciliation/AC/ADS/Comp). The old description only covered Part A/B/C, so user prompts hitting Part D scenarios or restaurant-BI questions weren't routing in. Also added a `# 🆎 Part D` stub section in the SKILL.md body so Part D has the same visibility as Part A/B/C/C-12. Fixed three V2.1.6 residual version drifts (main title V2.1.5→V2.1.7, openclaw npm constraint `^1.0.21→^1.0.24`, three-skill table `majia-guanyuan` row 2.1.5→2.1.7). Zero references / templates / command-surface changes.

**V2.1.6** (2026-05-21) — **NEW `references/v7-page-card-publish-pipeline.md`: V7 Page/Card publish pipeline + 3-state hard rules** (~340 lines). Distilled from an end-to-end "chain-coffee BI demo recording" production run (90 days / 1200 stores / 80K members / 20 tables / 17 ETLs / 6 HTML application dashboards) with 12 major pitfalls: ① v7 BI draft/release: `POST /api/page+/api/card` all fail (`60004 operation only allowed on draft page`), silver bullet is `guancli ≥ 1.0.24`'s bundled `guanvis-skill` one-shot `publish .`; ② CSV three-state rule — `会员ID IS NOT NULL` treats walk-ins as members, making the north-star indicator = 100% fake data; ③ STRING vs DATE type boundaries; ④ CSV boolean fields are `'TRUE'/'FALSE'` strings; ⑤ Spark CTE aliases must be English; ⑥ Window cannot nest in aggregate; ⑦ ETL update must carry `OUTPUT_DATASET.dataSource.dsId`; ⑧ no native API for dataset upload; ⑨ pandas to_csv 50× faster than to_excel; ⑩ global JOIN key naming; ⑪ off-white theme; ⑫ HTML SDK minimal skeleton. `@guandata/guancli` dep bumped `^1.0.21 → ^1.0.24` (now bundles public guanvis-skill).

**V2.1.5** (2026-05-18) — **NEW `references/restaurant-bi-formulas/`: restaurant-chain BI formula playbook** (10 markdown files, 2881 lines, fully anonymized). Distilled from two consecutive production analyst tenures + 39 production ETLs, covering: ① **Formula handbook** (7 chapters, 60+ SQL snippets) — date/time, customer/membership (RFM 8-class × marketing-action mapping + R-threshold tiers), revenue KPIs (AC/ADS/ADT/Comp/CRM%), channel/store (multi-channel review pipeline), coupon/discount, SQL utilities, data-quality traps. ② **6 ETL engineering patterns** (Ch 08) — 10-CTE DWD wide-table base / light-node-heavy-SQL vs heavy-node-light-SQL philosophy / financial dual-source reconciliation / POS system identification / member lifecycle multi-output / Cohort date×store grid. ③ **39 production ETL catalog index** (Ch 09) — categorized across 11 business domains + per-ETL node/IO/SQL quick reference + reuse decision matrix. SKILL.md routing table now includes restaurant BI formula entry. Zero breaking change, pure docs increment. Full details in [CHANGELOG.md](./CHANGELOG.md).

**V2.1.4** (2026-05-15) — Command surface synced with `@guandata/guancli@1.0.21`: ① **`metric query` generalized query** (1.0.20) — `--compare`, `--xtd`, `--recent`, `--percentage`, `--rank-top`, `--last`, fallback `--adv-calc-json`; ② **`card preview -f excel`** (1.0.20) — Excel 2003 XML export; ③ **Cleaner error output** (1.0.21). Manifest dep bumped `^1.0.19 → ^1.0.21`.
