...
[![Skill Version](https://img.shields.io/badge/skill-v3.1.8-blue)](./SKILL.md)
...
**Latest: V3.1.8** (2026-07-24) — **official family 07-15 + 07-24 two-batch alignment, folded into one release**. guanskill 0.1.12→**0.1.17**; net changes across all six sub-packages since v3.1.6 (07-10 batch): guanwf 0.1.7→**0.1.822** (the version jump is not a typo — the vendor switched to a build-number scheme at 07-15's 0.1.820 and moved to 0.1.822 at 07-24; verified on the npm registry, and semver-wise 82x > 7 so `@latest` still resolves correctly. **8.2.0 compatibility line, with a breaking change (introduced at 0.1.820)**: every write now sits behind a **`--confirm` gate** — `save`/`save-draft`/`run`/`schedule` require `--dry-run` to see the plan and `--confirm` to actually execute, so the old bare `guanwf save --dir X` no longer writes; plus workflow params `Workflow.Params` / `run --param` / event scheduling `schedule event`; strongly-typed task-node DSL (DATASET/HTTP/SQL/PARAMETER_ASSIGNMENT/SWITCH — SHELL/LOOP are unregistered on 8.2.0 and rejected by the CLI); instance diagnostics `instance list｜tasks｜logs｜latest`; and a **corrected failure-recovery semantic** — every DAG successor of the recovery node re-runs, including ones that previously succeeded; **new in 0.1.822**: faster workflow dependency/lineage analysis + in-place edit protection for existing workflows), guands 0.1.19→**0.1.23** (**`account rename`** renames a data account in place, preserving acId/permissions/credentials/dataset references and refusing delete-then-recreate + **`dataset primary-key set/clear`** as its own subcommand + `dataset refresh` checks status before submitting to avoid duplicate submissions and false success + **Excel multi-sheet now requires an explicit `--sheet`** + `import` waits for the task to reach a terminal state (all in 0.1.21); **new in 0.1.23**: data-connection config supports safe in-place updates (preview + validate first) + fill-in forms support export/edit/rename with existing fields and data preserved on update + fill-in form directory management), guancli 1.0.39→**1.0.43** (**`card preview` handles multi-level headers** — structured output now keys columns by the full `parent > child > leaf` path, making exports, column selection and sorting accurate on complex cross-tabs + filtered preview works even with restricted permissions, using the card's own field info + `form update` payloads must now carry every business primary-key field (all in 1.0.40); **new in 1.0.43**: more accurate token-login guidance and status checks so config/credential issues surface earlier + faster multi-resource reads for snappier Agent analysis of pages/datasets/metrics), guanvis 0.1.30→**0.1.33** (**`setFilterPanelLayout()`** configures filter-bar layout + `updateMetric`/`patchMetric` write-back **preserves untouched live config** + field display-name `alias` is independent of the card title + `--allow-overwrite` backup **no longer depends on upstream dataset permissions** (all in 0.1.31); **new in 0.1.33**: metric charts support primary/secondary metric groups + number grouping + pre-publish detection of invalid resources/unmounted cards/generation errors + faster complex-page editing and large-resource import), guanetl 0.1.19→**0.1.21** (unchanged at 07-15; **new in 0.1.21**: JOIN field-type checks extended from `export` to the full preview/save/run path + more accurate field recognition and multi-level upstream diagnosis, faster batch checks), guanmetric 0.1.1→**0.1.3** (unchanged at 07-15; **new in 0.1.3**: in-place edit protection for existing metrics + faster Agent batch preparation/validation of metric configs). Routing table refreshed across all 6 rows + **a coexistence note for the official `guandata-cli-suite` entry skill** (0.1.14 ships a first-party pure dispatcher; this table additionally carries version pins, capability inventory and field-tested boundaries) + architecture diagram version refresh + backfilled the marketplace/version-line/alt pins that v3.1.7 missed. Moat untouched.

**V3.1.7** (2026-07-12) — **restaurant BI formula playbook moved out to the standalone repo [majia-huiyuan](https://github.com/maojiebc/majia-huiyuan)**, merged with the coffee-chain simulated data platform (54 datasets / 25 ETLs / 12 dashboards, formerly examples/workshop513) into one "open-sourced membership-ops playbook" project with an AI-agent-friendly layer (llms.txt / AGENTS.md). Pointer READMEs remain at references/restaurant-bi-formulas/ and examples/; all SKILL.md formula routes now point to the new repo. Division of labor: **tools & pitfall handbooks in guanyuan, data & formulas in huiyuan**. Playbook migrated verbatim — moat untouched.
