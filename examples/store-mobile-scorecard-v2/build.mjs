...
const COUPONS = ['新人券', '满减券', '会员日券', '员工餐兑换券'];
const STATUS = [
  ['活跃（21 天内）', 0.42],
  ['低活（21–45 天）', 0.28],
  ['沉默（46–90 天）', 0.18],
...
function buildStore(name, spec) {
  ...
  // 每日合成订单：只把有效订单计入金额，叠券合计独立去重。
  const coupons = [];
  if (!spec.noCoupons) finDays.forEach((date, dayIndex) => {
    const blank = () => ({vouchers:0,orders:0,valid:0,paid:0,gross:0,discount:0,payment:0,unmatched:0,invalid:0,closed:0,pending_vouchers:0,stacked:0});
    const stats = COUPONS.map(blank), total = blank();
    function add(target, state, gross, paid, voucherCount, stacked) {
      target.vouchers += voucherCount;
      target.orders += state !== 'unmatched' ? 1 : 0;
      target.stacked += stacked ? 1 : 0;
      if (state === 'valid') { target.valid++; target.gross += gross; target.paid += paid; target.discount += gross-paid; }
      else { target[state] += state === 'unmatched' ? voucherCount : 1; target.pending_vouchers += voucherCount; }
    }
    const n = Math.round(spec.coupon * weekdayBoost(date));
    for (let i=0;i<n;i++) {
      const types = i === 0 ? [0,1] : [i % 4];
      const state = i===n-1 && dayIndex%3===0 ? 'unmatched' : types[0]===3 ? 'payment' : 'valid';
      const gross=30+(i%8)*5, paid=gross*(types[0]===0 ? .85 : .9);
      for (const t of types) add(stats[t],state,gross,paid,1,types.length>1);
      add(total,state,gross,paid,types.length,types.length>1);
    }
    stats.forEach((v,i) => {
      const {vouchers,...measure}=v;
      coupons.push({核销日期:date,券类型名称:COUPONS[i],核销券数:vouchers,
        转化明细:JSON.stringify({version:1,...measure,total:i===0?total:null})});
    });
  });
  const dormant = [
  ...
  if (spec.anomaly) {
    ...
    ctx['会员异常诊断']=JSON.stringify({version:1,kind:'同额高额订单重复',orders:9,days:9,amount:2880,share:.4,unit:320,
      threshold:200,cash_share:0,payment:'移动支付',product:'示例配料',start:addDays(ANCHOR,-29),end:ANCHOR});
  }
  for (const way of ['堂食','外卖']) {
    const base=fin.filter(x=>x.取餐方式===way && x.订单日期>=addDays(ANCHOR,-6)).reduce((n,x)=>n+x.订单数,0);
    const valid=spec.anomaly && way==='堂食' ? 18 : Math.round(base*.9);
    ctx[way+'汤底订单基数']=base; ctx[way+'有效汤底订单']=valid;
    let left=valid;
    soup.filter(x=>x.取餐方式===way).forEach((x,i)=>{x.订单数=i===4?left:Math.round(valid*[.32,.25,.2,.14][i]);left-=x.订单数;});
  }
  const fixedRows=newold.filter(x=>x.订单日期>=addDays(ANCHOR,-29));
  const orders=fixedRows.reduce((n,r)=>n+r.订单数,0), memberOrders=fixedRows.filter(x=>x.是否会员==='会员').reduce((n,r)=>n+r.订单数,0), ratio=memberOrders/orders;
  Object.assign(ctx,{'会员对比开始日':addDays(ANCHOR,-29),'会员对比截止日':ANCHOR,'会员对比占比':ratio,
    '会员对比订单数':orders,'会员对比会员订单数':memberOrders,'会员对比有效天数':30,'会员对比缺失天数':0,
    '会员对比识别覆盖率':spec.noCoupons?.65:.94,'会员对比状态':spec.noCoupons?'识别覆盖不足':'可参评',
    '会员对比前期占比':ratio-.025,'会员对比前期状态':spec.noCoupons?'识别覆盖不足':'可参评','会员对比变化':.025});
  ...
  const named = {
    ...
    coupons: toCols(coupons, ['核销日期', '券类型名称', '核销券数', '转化明细']),
    dormant: toCols(dormant, ['复购状态', '顾客数']),
    context: toCols(context, Object.keys(ctx)),
...
const specB = {
  anomaly: true, seed: 77, base: 11800, yesterday: 15120, dineShare: 0.54, ac: 31.2, newShare: 0.19,
...
const viewsC = buildStore(STORE_C,{...specA,seed:93,base:4600,yesterday:5200,branch:'示例北区',type:'社区店',noCoupons:true});
// 同店营业额排行与首屏保持一致。
for (const v of [viewsA,viewsB,viewsC]) {
 const p=v[detectV(v).peer], names=p.find(c=>c.name==='门店名称').data, rev=p.find(c=>c.name==='营业额').data;
 for (const [name,views] of [[STORE_A,viewsA],[STORE_B,viewsB],[STORE_C,viewsC]]) rev[names.indexOf(name)]=Math.round(avg7(views)*7);
}
// 以同一组固定窗口数据计算三种对比，所有切换门店共用同一同店样本。
const demoViews={[STORE_A]:viewsA,[STORE_B]:viewsB,[STORE_C]:viewsC};
const contexts=Object.fromEntries(Object.entries(demoViews).map(([n,v])=>[n,Object.fromEntries(v[detectV(v).context].map(c=>[c.name,c.data[0]]))]));
const memberCohort=PEERS.filter(p=>!contexts[p.name] || contexts[p.name]['会员对比状态']==='可参评').map((p,i)=>({...p,ratio:contexts[p.name]?.['会员对比占比']??(.12+(i%9)*.035)}));
for(const [name,v] of Object.entries(demoViews)) {
 const ctx=contexts[name], ratio=ctx['会员对比占比'];
 if(ctx['会员对比状态']!=='可参评') continue;
 for(const [label,key,field] of [['分公司','branch','分公司'],['城市','city','地理城市'],['店型','type','门店类型']]) {
  const group=memberCohort.filter(p=>p[key]===ctx[field]).sort((a,b)=>a.ratio-b.ratio),n=group.length;
  const rank=1+group.filter(p=>p.ratio>ratio).length,q=rank/n;
  ctx['会员对比'+label+'名次']=rank;ctx['会员对比'+label+'中位']=(group[Math.floor((n-1)/2)].ratio+group[Math.floor(n/2)].ratio)/2;
  ctx['会员对比'+label+'分位']=q<=.1?'前10%':q<=.25?'前25%':q<=.5?'前50%':q<=.75?'后50%':'后25%';
 }
 v[detectV(v).context]=toCols([ctx],Object.keys(ctx));
}
...
if (mapA.rfm !== -1) throw new Error('V2 不取 RFM，detectV.rfm 必须是 -1');
...
const fixture = {
  today: TODAY,
  storeNames: [STORE_A, STORE_B, STORE_C],
  stores: {
    [STORE_A]: { branch:'示例南区', revenue7:Math.round(avg7(viewsA)*7), views:viewsA },
    [STORE_B]: { branch:'示例南区', revenue7:Math.round(avg7(viewsB)*7), views:viewsB },
    [STORE_C]: { branch:'示例北区', revenue7:Math.round(avg7(viewsC)*7), views:viewsC },
  },
};
...
<body>
<div class="demo-banner">脱敏演示 · 虚构门店与虚构数字 · 数据锚点 2026-03-20 · 双击即可离线点 · V2 · 不含真实品牌 / 门店 / 金额</div>
<div class="demo-wrap"><div class="demo-filters"><label>分公司<select id="demo-branch"></select></label><label>门店 · 近7天营业额降序<select id="demo-store"></select></label></div><div id="app"></div></div>
<script>
